...
int main(int argc, char **argv)                         // 노드 메인 함수
{
  ...
  ros::Publisher ros_cmd_vel_pub = nh.advertise<cmd_vel_topic::msgCmdVel>("cmd_vel", 100);
  ...
  msg.cmd_vel_twist.linear.x = 0;
  msg.cmd_vel_twist.angular.z = 0;
  while (ros::ok())
  {
    msg.data = count;                   // count 라는 변수를 이용하여 메시지 값을 정한다

	std::cout << "input velocity" << std::endl;
	std::cin >> msg.cmd_vel_twist.linear.x >> msg.cmd_vel_twist.angular.z ;
